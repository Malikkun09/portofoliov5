...
function ButtonLink({ href, label, target = false, compact = false }) {
  ...
  return (
    <Link target={target ? '_blank' : undefined} rel={target ? 'noopener noreferrer' : undefined} aria-label={label} scroll={false} href={href}>
      <button type="button" aria-label={label} ref={buttonRef} className={clsx('p-xs', styles.btnPosnawr, compact && styles.compact)} onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
